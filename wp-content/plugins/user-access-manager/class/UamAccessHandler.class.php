...
//    public function getUserGroups($userGroupId = null, $filter = true)
    public function getUserGroups($userGroupId = null, $filter = false)
    {
        if ($filter) {
            $filterAttr = 'filtered';
        } else {
...
//        );
        $userGroupsDb = $wpdb->get_results(
        	"SELECT ID
        	FROM " . DB_ACCESSGROUP . "
        	ORDER BY groupname ASC", ARRAY_A
        );
...
//    public function getUserGroupsForObject($objectType, $objectId, $filter = true)
    public function getUserGroupsForObject($objectType, $objectId, $filter = false)
    {
...
//         $postableTypes = $this->getPostableTypes();
//         $postableTypesString = "'".implode("','", $postableTypes)."'";
//         // get posts that are assigned to the users groups OR are not assigned to ANY group (RIGHT join)
//         $postAssignedToUserSql = "
//         	 SELECT igp.object_id
//         	 FROM ".DB_ACCESSGROUP_TO_OBJECT." AS igp
//         	 RIGHT JOIN $wpdb->posts AS p ON igp.object_id=p.ID
//         	 WHERE igp.object_id IS NULL OR 
//         	 	(igp.group_id IN (".$userUserGroupString.") 
//          			AND igp.object_type IN ( ".$postableTypesString." ) )";
//    die($postAssignedToUserSql);    
  ...
  //right join wp_dgo_posts on object_id=ID
